...
<nav class="container-fluid">
  <ul><li><strong>ネットスーパー価格</strong></li></ul>
  <ul>
    <li><a href="select.html">商品選択</a></li>
    <li><a href="#">お問い合わせ</a></li>
    <li><a href="#" role="button">ログイン</a></li>
  </ul>
</nav>
...
<main class="container">
      ...
      <p>最終更新日時：<?php echo htmlspecialchars($prices[0]['updated_at']); ?></p>
  ...
  <div class="container">
    <article>
      ...
      <form class="grid">
        ...
        <button type="submit" onclick="event.preventDefault()">購読</button>
      </form>
    </article>
  </div>
</section>
...
<footer class="container">
  <small><a href="#">利用規約</a> • <a href="#">プライバシー</a></small>
</footer>
...
<script>
  ...
  function preloadImages(callback) {
    ...
    container.style.width = "25vw"; // 先に幅を固定

    const tempWrapper = document.createElement("div");
    tempWrapper.style.position = "absolute";
    tempWrapper.style.visibility = "hidden";
    tempWrapper.style.zIndex = "-9999";
    tempWrapper.style.width = "40vw";
    document.body.appendChild(tempWrapper);

    imagePaths.forEach((path) => {
      const img = new Image();
      img.src = "img/" + path + "?v=" + Date.now();
      img.style.width = "100%"; // ← wrapperに合わせる
      img.onload = function () {
        tempWrapper.appendChild(img);

        // 強制リフローさせる（DOMに描画された後）
        const height = img.getBoundingClientRect().height;
        if (height > maxDisplayHeight) maxDisplayHeight = height;

        tempWrapper.removeChild(img);
        loaded++;

        if (loaded === imagePaths.length) {
          document.body.removeChild(tempWrapper); // wrapper 削除
          container.style.height = maxDisplayHeight + "px";
  ...
  window.onload = updateImage;
    preloadImages(() => {
    initializeSlider(); // スライダーの初期化
    drawChart();        // グラフの描画（高さが確定した後）
  });
</script>
</body>
</html>
